...
async def retrieve_water(day: int):
    water = await water_collection.find_one({"Day": int(day)})
    if water:
        return water_helper(water)


# Update a water with a matching ID
async def update_water(day: int, data: dict):
...
async def delete_water(day: int):
    water = await water_collection.find_one({"Day": int(day)})
    if water:
        await water_collection.delete_one({"Day": int(day)})
        return True
